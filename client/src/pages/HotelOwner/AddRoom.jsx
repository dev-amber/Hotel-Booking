...
const AddRoom = () => {
  ...
  return (
    <form>
        ...
        {Object.keys(images).map((key) => (
          <label htmlFor={`roomImage-${key}`} key={key}>
            <img
              ...
              alt={`Room upload preview ${key}`}
              className="h-32 w-32 object-cover rounded cursor-pointer opacity-90"
            />
            <input
              type="file"
              accept="image/*"
      ...
      <button className="bg-primary text-white px-8 py-2 rounded mt-4 mb-10">
        Add Room
      </button>
    </form>
  )
