...
const HotelCard = ({room, index}) => {
  return (
    <Link to={"/rooms/" + room._id} onClick={() => scrollTo(0,0)} key={room.id}
    className='relative max-w-70 w-full rounded-xl overflow-hidden
     bg-white text-gray-500/90 shadow-[0px_4px_rgba(0,0,0,0.5]'
    >
       <img src={room.images[0]} alt='' /> 
        
        {/* we want this seller have selected cards apply */}
       { index % 2 == 0 && <p className='px-3 py-1 absolute top-3 left-3 text-xs bg-white text-gray-800
