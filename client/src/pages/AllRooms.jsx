...
const AllRooms = () => {
    ...
    {/* to present data in map form and checkbox*/}
    const roomTypes=[
        "Single Bed",
        "Double Bed",
        "Luxury Room",
  ...
  return (
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between
     pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        {/* left side hotels display */}
      <div>
        <div className='flex flex-col items-start text-left'>
            <h1 className='font-playfair text-4xl md:text-[40px]'>Hotel Rooms</h1>
            <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>
            Take advantage of our limited-time offers and special packages 
            to enhance your stay and create unforgettable memories</p>
        </div>

        {roomsDummyData.map((room)=> (
         <div  key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b
          border-gray-300 last:pb-300 last:border-0'>
            <img src={room.images[0]} alt='hote;-img' title='View Room Details'
            className='max-h-65 md:w-1/2 roundex-xl shadow-lg object-cover cursor-pointer'
      ...
      <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16 '>
         <div className={`flex items-center justify-between px-5 py-2.5 min-lg:border-b
             border-gray-300 ${openFilters && "borderb"}`}>
